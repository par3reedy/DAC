...
uint16_t DAC_volt_conv(uint16_t keypad_volts){
    ...
    if (keypad_volts >= 329){                       //If user input is greater than 330 set output to max voltage
        ...
        //optimization = (124410*keypad_volts+43724); //Optimization Function derived from linear optimization
        //volts = optimization/10000;                 //Convert back to units for DAC
        optimization = (12467*keypad_volts-659);
        volts = optimization/1000;
        return volts;
    }
}
